...
if (isset($_POST['submit'])) {
	...
	if($name){
		...
		$lq="INSERT INTO documments VALUES('','$doc_name','$location','$date','$row[first_name] $row[last_name]','chem1113')";
...
if (isset($_POST['submit1'])) {
	...
	$date = date('Y-m-d H:i:s');
		...
		$lq="INSERT INTO documments VALUES('','$doc','$docu','$date','$row[first_name] $row[last_name]','chem1113')";
		$sq=mysqli_query($con,$lq);
		// header('location:index1.php');
...
<body>
	...
	<div class="part">
   	    <ul>
			...
			<i class="fa fa-circle-o" aria-hidden="true" style="font-size:15px;color:#3A6E96;"></i>
			<i class="fa fa-circle-o" aria-hidden="true" style="font-size:15px;color:#3A6E96;"></i></div>
   	    	<div class="bb"><li class="active"><a href="chem1113video.php"><i class="fa fa-laptop" aria-hidden="true" style="font-size:35px;"></i></a><br />Video/Slide</li></div>
			<div class="b"><i class="fa fa-circle-o" aria-hidden="true" style="font-size:15px;color:#3A6E96;"></i>
			<i class="fa fa-circle-o" aria-hidden="true" style="font-size:15px;color:#3A6E96;"></i>
			<i class="fa fa-circle-o" aria-hidden="true" style="font-size:15px;color:#3A6E96;"></i></div>
   	    	<div class="bb"><li ><a href="chem1113question.php"><i class="fa fa-question aria-hidden="true" style="font-size:38px;"></i></a><br />Question</li></div>
...
<center><h2>Inorganic and Physical Chemistry</h2></center>
  <br /> <h1>UPLOAD</h1>
	<form name="file" enctype="multipart/form-data" method="post" action="chem1113video.php" class="main"> 
<input type="text" name="doc_name" placeholder="Topics Name" />
<input type="file" name="myfile" />
<input type="submit" name="submit" value="SUBMIT" />
</form>
<h1>GIVE THE LINK</h1>
<form  method="post" action="chem1113video.php" class="main"> 
...
<br><br>
  ...
  if(isset($_POST['new_comment'])) {
    ...
    if(isset($new_com_text)) {
      mysqli_query($con, "INSERT INTO parents1 ( user,text, date, code,course) VALUES ('$row[first_name] $row[last_name]','$new_com_text', '$new_com_date', '$new_com_code','chem1113')");
    }
  ...
  if(isset($_POST['new_reply'])) {
    ...
    if(isset($new_reply_text)) {
      mysqli_query($con, "INSERT INTO children1 (user, text, date, par_code,course) VALUES ('$row[first_name] $row[last_name]','$new_reply_text', '$new_reply_date', '$new_reply_code','chem1113')") or die(mysqli_error());
    }
...
?>
    ...
    require '../../config.php';
     $result1 =  mysqli_query($con, "SELECT * FROM documments WHERE course='chem1113' ORDER BY id DESC");
    ...
    $result =  mysqli_query($con, "SELECT * FROM parents1 WHERE code='$par_code' and course='chem1113' ORDER BY date DESC");
    ...
    foreach($result as $item) {
      ...
      echo '<div class="comment" id="'.$par_code.'">'
        ...
        $chi_result = mysqli_query($con, "SELECT * FROM children1 WHERE par_code='$par_code' and course='chem1113' ORDER BY date DESC");
